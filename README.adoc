...
steps:
  ...
  - uses: MMProgrami/after-maven-action@v2
    ...
    with:
      ...
      maven_outcome: ${{ steps.maven.outcome }}
  - name: Use variable
    run: |
...
`maven_outcome` also makes the action fail when Maven itself fails, including compilation errors. `continue-on-error: true` and `if: always()` ensure that the report step runs after such a failure.
...
maven_outcome:: The outcome from the preceding Maven step (default `success`), normally `${{ steps.maven.outcome }}`. A `failure` value makes this action fail, even when no test report was produced.
...
It will also concisely output which tests fail, if there are any.
...
Originally this script was using bash/xsltproc, and was (and is) used in link:https://github.org/vpro/maven[a dedicated maven docker image], which we use in gitlab pipelines. To make it usable as a GitHub Action too, I started with just installing the necessary tools and run the same script. This makes it a bit slow as a GitHub Action. So starting from v2 it is purely Node.js, just based on the same XSLTs (using SaxonJS to execute them)
