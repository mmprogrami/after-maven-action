...
steps:
  ...
  - uses: MMProgrami/after-maven-action@v2
    ...
    with:
      ...
      maven_exit_code: ${{ steps.maven.outcome == 'failure' && 1 || 0 }}
  - name: Use variable
    run: |
...
`maven_exit_code` also makes the action fail when Maven itself fails, including compilation errors. `continue-on-error: true` and `if: always()` ensure that the report step runs after such a failure. The example maps a failed Maven step to exit code `1`; if the exact Maven exit code is required, write it to a step output from a shell that captures `mvn`'s status.
...
maven_exit_code:: The exit code from the preceding Maven build (default `0`). A non-zero value makes this action fail, even when no test report was produced.
